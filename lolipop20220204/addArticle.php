...
if(isset($_POST["formPostName"]) && isset($_POST["formPostWork"]) && isset($_POST["formPostWorkPseudonym"]) && isset($_POST["formPostWorkType"]) && isset($_POST["selectPrefecture"]) && isset($_POST["formPostMunicipality"]) && isset($_POST["formPostLast"])) {
    ...
    $sql = "
        ...
        WHERE 
            ...
            type = ?;
    ";
    $workID = execsql($conn, $sql, array($work, $type))->fetch()["workID"];
    ...
    try {
        ...
        $sql = "
            ...
            VALUES(?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?);
        ";
        execsql($conn, $sql, array($userID, $workID, $filename, $name, $sAddress1, $sAddress2, $sAddress3, $details, 0, 0, $date));

        $sql = "
            SELECT LAST_INSERT_ID() 
            AS id;
        ";
        $aid = execsql($conn, $sql)->fetch()["id"];
