window.onload = () => {
    ...
    buttonGo.onclick = () => {
        ...
        if(pushed) {
            // 非同期でunpush_go.phpを実行
            let request = new XMLHttpRequest();
            request.open("GET", `./unpush_go.php?aid=${ aid }`, true);
            ...
            // 非同期でpush_go.phpを実行
            let request = new XMLHttpRequest();
            request.open("GET", `./push_go.php?aid=${ aid }`, true);
            request.responseType = "json";
            request.onload = response => {
                let result = response.target.response;
    ...
    buttonFavorite.onclick = () => {
        ...
        if(pushed) {
            // 非同期でunpush_go.phpを実行
            let request = new XMLHttpRequest();
            request.open("GET", `./unpush_favorite.php?aid=${ aid }`, true);
            request.responseType = "json";
            request.onload = response => {
                let result = response.target.response;
            ...
            // 非同期でpush_go.phpを実行
            let request = new XMLHttpRequest();
            request.open("GET", `./push_favorite.php?aid=${ aid }`, true);
            request.responseType = "json";
            request.onload = response => {
                let result = response.target.response;
